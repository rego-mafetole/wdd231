...
// THE COMPLETE COURSE DATA ARRAY
...
function displayCourses(filteredCourses) {
    ...
    counter.textContent = filteredCourses.length;
}
...
document.getElementById("btn-wdd").addEventListener("click", (e) => {
    toggleActive(e.target);
    const wddOnly = courses.filter(course => course.type === "WDD");
    displayCourses(wddOnly);
});

document.getElementById("btn-cse").addEventListener("click", (e) => {
    toggleActive(e.target);
    const cseOnly = courses.filter(course => course.type === "CSE");
    displayCourses(cseOnly)
});

function toggleActive(activeBtn) {
    document.querySelectorAll(".filter-buttons button").forEach(btn => btn.classList.remove("active-btn"));
